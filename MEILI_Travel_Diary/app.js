...
passport.use(new LocalStrategy(
    function(username, password, done) {
        ...
        var prioryQuery = myClient.query("SELECT login_user as id FROM raw_data.login_user($1,$2)", [username, password]);
...
passport.use(new UniqueTokenStrategy(
	function (token, done) {
        ...
        //call database for login
        var prioryQuery = myClient.query("SELECT id, username FROM raw_data.user_table WHERE device_session = $1", [token]);
		...
		prioryQuery.on('end', function(){
			...
			if (results[0].id){
				//retrieved id successfully
				done(null, {userId: results[0].id, userName: results[0].username});
			}
			else
			{
				//failed to login
...
var auth = function (req, res, next) {
    if (!req.isAuthenticated()) {
		...
		myClient.query("UPDATE raw_data.user_table SET last_seen = now() "+
                                     "WHERE id=$1",
