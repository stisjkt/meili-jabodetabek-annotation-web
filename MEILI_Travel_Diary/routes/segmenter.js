...
module.exports = {
    ...
    generateTrips: function (userId) {
		...
		pool.query("UPDATE raw_data.user_table SET last_seen = now() "+
                                     "WHERE id=$1", [userId]);
        ...
        var pointsInActiveTrip = 0;
            ...
            pool.query("select get_stream_for_stop_detection as response from apiv2.get_stream_for_stop_detection($1);",[userId],
            function(err, result){
                                 ...
                                 if (!skipOne) {
                                     ...
                                     if (stopNumber >= 1) {
                                         ...
                                         if ((endLocation.time_ - firstLocation.time_) >= 5 * 60000 && pointsInActiveTrip >= 4) {
                                             ...
                                             if (activeTrip.from_time != activeTrip.to_time && passiveTrip.from_time != passiveTrip.to_time && prevFrom.time_ >= points[0].to_time)
                                             {
	...
	clearInvalidTripsExposed : function (userId) {
		pool.query("select id from raw_data.user_table;", function(err, result){
			for (var j in result.rows){
				console.log('clearInvalidTrips: ' + result.rows[j]['id']);
				clearInvalidTrips(result.rows[j]['id']);
			}
		});
	}
};

function clearInvalidTrips(userId) {
	pool.query("select trip_id from apiv2.trips_inf where trip_id not in (select trip_id from apiv2.triplegs_inf) and user_id =($1) order by from_time desc;",[userId], function(err, result){
		for (var j in result.rows){
			console.log('delete trip: ' + result.rows[j]['trip_id']);
			pool.query("select apiv2.remove_trip("+ result.rows[j]['trip_id'] +");", function(err, result){
				if(err){
...
function generateTriplegs(userId) {
    ...
    var results = [];
        ...
        pool.query("select get_stream_for_tripleg_detection as response from apiv2.get_stream_for_tripleg_detection($1);",[userId], function(err, result){
...
function generateTriplegSql(arrayOfTriplegs) {
    ...
    for (var i=0; i<triplegs.length;i++){
        var user_id = triplegs[i].user_id;
		var values = [];
        var object =[];
		...
		pool.query(sql + "values " + values.toString(), function(err, result){
			results.push('done');
			if(results.length >= triplegs.length){
				clearInvalidTrips(user_id);
			}
            if (err){
...
function conditionIsMet(startLocation, endLocation, centroidLat, centroidLon, divisionNumber) {
    ...
    if (centroidLat!=0 && centroidLon!=0){
        if (Math.max(calcCrow(endLocation.lat_, endLocation.lon_, centroidLat/divisionNumber, centroidLon/divisionNumber),calcCrow(startLocation.lat_, startLocation.lon_, endLocation.lat_, endLocation.lon_))<=50) distanceIsOk=false;
    }
